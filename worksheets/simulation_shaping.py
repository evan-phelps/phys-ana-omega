...
# # simulation shaping
# 
# To shape the simulated data to realistic distributions, assume GSIM is good -- determine shaping factors from comparison of reconstructed simulation and experimental distributions normalized by virtual photon flux and background-subtracted.  Mandelstam $t$ or $cos(\theta)$ has the largest impact. $\phi$ will remain unchanged for now.  Both simulation and experiment will have all cuts applied.
# 
# ## setup functions and histograms
...
fig = draw(hq2w)
for wedge in arange(1.6,3.2,0.05):
    axvline(wedge, color='k')
for q2edge in arange(1.0, 5.5, 4.5/9):
    axhline(q2edge, color='k')
...
hms = get_projs(hmq2w)
fout = ropen('bgfits.root','recreate')
...
def drawpub(n, hgbs, i):
    rplt.errorbar([results[n][2]], xerr=False)
    xlabel(r'MM$_\textrm{p}$ (GeV)', fontsize=14)
    yticks(arange(2.5e6,ylim()[1],2.5e6))
    #ylim(0,2e7)
    xticks(arange(0.4, 1.21, 0.1))
    if i == 1: ylabel(r'normalized yield (arbitrary units)', fontsize=14)
    grid()
