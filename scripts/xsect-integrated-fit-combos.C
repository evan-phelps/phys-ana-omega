...
TF1* getBgFn(double fit_x0, double fit_x1,
	     double step_x0, double step_x1,
	     double wval, int special = 0) {
  TF1 *fbg = f_pol4("fbg",fit_x0,fit_x1,true);
  fbg->FixParameter(7,step_x0);
  fbg->FixParameter(8,step_x1);
  if (special==1 && wval<2.2) {
    fbg->FixParameter(3,0);
    fbg->FixParameter(4,0);
  }
}

TF1* getSigFn(double fit_x0, double fit_x1,
	      double step_x0, double step_x1,
	      int special = 0) {
  TF1 *fsig = new TF1("fsig",&d_sig,fit_x0,fit_x1,5);  
  fsig->FixParamter(1,0.783);
  fsig->FixParamter(2,-0.0065+0.013*wval);
  fsig->SetParameter(3,step_x0);
  fsig->SetParameter(4,step_x1);
}

TF1* getFullFn(double fit_x0, double fit_x1,
	       double step_x0, double step_x1,
	       double wval, int special = 0,
	       TF1 *fbg2, TF1 *fgaus) {
  TF1 *f = f_pol4gaus("f",fit_x0, fit_x1 ,fbg2,fgaus);
...
void fitmmp(TH1 *hmmp, float step_x0 = 2, float step_x1 = 2.1, float wval = 0, bool verbose = false) {
  ...
  if (true) { //gMinuit->fCstatu.Contains("CONVER")) {
    ...
    //if (step_x0<pol2cutoff) {
    //if (wval>0) fbg->SetParameter(2,bgp2);
    if (wval<2.2) {
      //fbg2->FixParameter(2,bgp2);
    ...
    //fgaus->SetParLimits(2,0.015,0.032);
    float gmax = 0.85;
