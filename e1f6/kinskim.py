...
def usage():
    print(
        'kinskim [-f] [-w <workingdir>] [-c <parmsfilepath>] [-i <inputstring>] [-N <numtoproc>] [-t treepath] [-o outfile]')
...
def main(argv):
    ...
    for dep in ['Config.cpp', 'DataHandler.h', 'HandlerChain.cpp', 'H10.C',
                ...
                'DH_SC_Hists_PrePid.h', 'DH_CC_Hists.h', 'DH_KinTree.h']:
        r.gROOT.ProcessLine('.L %s/%s+' % (wdir, dep))

    handlers = [("h10clone", r.DH_CloneH10),
                ("tkin", r.DH_KinTree)]
