...
class DH_MMp_Exclusive : public DataHandler
{
    public:
        ...
        DH_MMp_Exclusive(std::string name = "DH_MMp_Exclusive", TDirectory *pDir = NULL, H10 *h10looper = NULL) : DataHandler(name, pDir, h10looper)
        {
            fDir->cd();
        }
        ...
        virtual bool Handle(H10* d)
        {
            ...
            if ( d->npip == 1 && d->MMppip < 0 ) return false;
            if ( d->npim == 1 && d->MMppim < 0 ) return false;
